...
class DepthRamTrap final : public Trap {
public:
    explicit DepthRamTrap(Vector2 trapCenter);
    void draw(float cameraX, bool highlighted) const override;
    bool hitsPlayer(const PlayerBall& player) const override;

private:
    float ramDepth() const;
    Rectangle hitRect() const;
};

class DropTileTrap final : public Trap {
public:
    explicit DropTileTrap(Vector2 trapCenter);
    void draw(float cameraX, bool highlighted) const override;
    bool hitsPlayer(const PlayerBall& player) const override;

private:
    float openAmount() const;
    Rectangle gapRect() const;
};

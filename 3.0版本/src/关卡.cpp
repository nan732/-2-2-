...
void Level::reset()
{
    ...
    traps.emplace_back(std::make_unique<PushDoorTrap>(Vector2{2520.0f, groundY}));
    traps.emplace_back(std::make_unique<TeeterRockTrap>(Vector2{3260.0f, groundY}));
}
...
bool Level::triggerNearestTrap(PlayerBall& player)
{
    for (const auto& trap : traps) {
        if (trap->isBusy()) {
            return false;
    ...
    Trap& trap = *traps[static_cast<size_t>(index)];
    const bool triggered = trap.trigger();
    if (triggered) {
        trap.onTriggered(player);
    }
    return triggered;
}
